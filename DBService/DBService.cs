...
/// <summary>
/// 2022.07.11 : parameter Value에 DbNull 넣는 부분 추가
/// </summary>
    ...
    private void SetParaValue(IDbCommand dbCommand, MyCommand myCmd, int iSetSeq)
    {
        ...
        foreach (IDbDataParameter para in dbCommand.Parameters)
        {
                ...
                para.Direction == ParameterDirection.InputOutput )
            {
                ...
                // 2022.07.11 추가 : parameter value = null 실행 시 오류가 남
                if (para.Value == null) para.Value = System.DBNull.Value;
            }
            iCol++;
                ...
                ((IDbDataParameter)dbCommand.Parameters[para.ParameterName]).Direction == ParameterDirection.InputOutput)
            {
                ...
                DBOutPut? output = OutputList.OrderByDescending(x => x.Rowseq)
                                                .FirstOrDefault(x => x.CommandName == para.HeaderCommandName &&
                                                                    x.ParameterName == para.HeaderParameter);
...
{
    private ConnectDB ConnectValue { get; set; }
    ...
    public DBFillManager(MyCommand myCmd)
    {
        try
        {
            ...
            for (int i = 0; i < myCmd.Parameters?.Length; i++)
            {
                ...
                IDbDataParameter? dbDataParameter = MyDbStatic.CreateParameter(myCmd.Parameters[i], ConnectValue.ProviderName);
    ...
    public string ExecDBFill(IDbCommand dbCommand)
    {
        ...
        try
        {
            if (ConnectValue == null) throw new Exception("DB Fill Manager DB Connection string Error");
                
            // DB Adapter
            IDataAdapter? dbAdapter = MyDbStatic.CreateDbAdapter(dbCommand, ConnectValue.ProviderName);
    ...
    public DBExecManager(MyCommand[] myCmds)
    {
        try
        {
            ...
            foreach (MyCommand myCmd in myCmds)
            {
                ...
                ConnectDB connectValue = Program.MyConfiguration.GetSection(strSection).Get<ConnectDB>();


                IDbConnection? dbconnection = MyDbStatic.CreateConnection(connectValue);
...
//                ConnectionStringSettings settings = ConfigurationManager.ConnectionStrings[myCmd.ConnectionName];
                ...
                if (connectValue.ProviderName.Equals("Oracle.ManagedDataAccess.Client"))
...
{
    public static IDbConnection? CreateConnection(ConnectDB connectVal)
    {
        if (connectVal.ProviderName.Equals("System.Data.SqlClient"))
            return new SqlConnection(connectVal.ConnectionString);
        else if (connectVal.ProviderName.Equals("Oracle.ManagedDataAccess.Client"))
            return new OracleConnection(connectVal.ConnectionString);
        else if (connectVal.ProviderName.Equals("MySql.Data.MySqlClient"))
            return new MySqlConnection(connectVal.ConnectionString);
        else
            return null;
    }
